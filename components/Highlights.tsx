      ...
      <section className="bg-transparent px-4 flex flex-col items-center relative pt-0 md:pt-6">
        <div className="w-full max-w-5xl mb-8">
          <div className="flex flex-row items-start justify-between py-16">
            <div>
              <h2 className="text-3xl font-medium mb-4 text-left text-[--foreground]">
                {highlightsData.title}
              </h2>
              <span className="inline-block border-2 border-[--blue50] text-[--blue50] rounded-full px-5 py-1 text-xl font-semibold mb-4 md:hidden">
                {highlightsData.year}
              </span>
            ...
            <div className="ml-8 mt-1">
              <span className="hidden md:inline-block border-2 border-[--blue50] text-[--blue50] rounded-full px-5 py-1 text-xl font-semibold">
                {highlightsData.year}
              </span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-md md:max-w-5xl mt-12">
          {highlightsData.grid.map((img, idx) => (
            <div key={idx} className="relative">
              <div className="w-full aspect-square md:aspect-[3/2] bg-[--gray20] dark:bg-[--gray60] flex items-center justify-center overflow-hidden">
                <img
                  src={img.src}
                  alt={img.alt}
