  ...
  return (
    <footer className="pt-12 sm:pt-16 pb-6 px-2 sm:px-4">
      <div className="flex flex-col items-center">
        ...
        />
        <h2 className="text-3xl md:text-5xl font-extralight text-center mb-4 text-[--foreground]">
          {footerData.footerText.heading}
        </h2>
        <p className="text-center text-[--muted-foreground] max-w-2xl mb-2">
          {footerData.footerText.description}
        </p>
        <SocialLinks links={footerData.socialLinks} />
        <div className="w-11/12 sm:w-10/12 mx-auto border-y border-[--border] py-16">
            ...
            <div>
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <div className="flex flex-col text-[--muted-foreground] text-base space-y-3 mt-4 z-[99]">
                    {footerData.about.map((item) => (
                      <Link
                        key={item.link}
                        href={item.link}
                        className="hover:text-[--blue50] transition-colors cursor-pointer"
                      >
                        {item.name}
                      </Link>
                    ))}
                  ...
                  <div className="flex flex-col text-[--muted-foreground] text-base space-y-3 mt-4 z-[99]">
                    {footerData.resources.map((item) => (
                      <Link
                        key={item.link}
                        href={item.link}
                        className="hover:text-[--yellow50] transition-colors cursor-pointer"
                      >
                        {item.name}
                      </Link>
                    ))}
                  ...
                  <div className="flex flex-col text-[--muted-foreground] text-base space-y-3 mt-4 z-[99]">
                    {footerData.legal.map((item) => (
                      <Link
                        key={item.link}
                        href={item.link}
                        className="hover:text-[--green50] transition-colors cursor-pointer"
                      >
                        {item.name}
                      </Link>
                    ))}
        ...
        <div className="flex flex-col md:flex-row justify-between items-center w-11/12 sm:w-10/12 mx-auto py-6 sm:py-8">
          ...
          <span className="text-xs text-[--muted-foreground] text-center md:text-right w-full md:w-auto">
